...
"""Enhanced unit tests for vdc_certificate module with 100% coverage."""
...
MODULE = 'ansible_collections.dellemc.objectscale.plugins.modules.vdc_certificate'
...
def _run_module(params=None, check_mode=False, diff=False):
    """Run the vdc_certificate main() with mocked AnsibleModule and API."""
    from ansible_collections.dellemc.objectscale.plugins.modules.vdc_certificate import main
    ...
    with patch(f'{MODULE}.AnsibleModule', return_value=module_mock), \
         ...
         patch(f'{MODULE}.VdcKeystoreApi', return_value=keystore_api_mock):
        yield module_mock, keystore_api_mock, safe_run


class TestVdcCertificateIdempotency:
    ...
    def test_idempotent_returns_details(self):
        """Idempotent result includes vdc_certificate_details."""
        with _run_module() as (module, api, run):
            api.get_certificate_chain.return_value = {'chain': SAMPLE_CHAIN_PEM}
            ...
            assert kwargs['vdc_certificate_details']['chain_length'] >= 1


class TestVdcCertificateCheckMode:
...
class TestVdcCertificateDiffMode:
...
class TestVdcCertificatePemValidation:
    """Test PEM validation."""
    ...
    def test_valid_pem_formats(self):
        """Test various valid PEM formats."""
        with _run_module() as (module, api, run):
            api.get_certificate_chain.return_value = {'chain': SAMPLE_CHAIN_PEM}
...
class TestVdcCertificateFilePath:
...
class TestVdcCertificateErrorHandling:
    """Test error handling."""
    ...
    def test_500_server_error(self):
        ...
        with _run_module() as (module, api, run):
            ...
            kwargs = module.fail_json.call_args[1]
            assert 'Failed to set VDC certificate' in kwargs['msg']
    ...
    def test_get_chain_generic_error(self):
        ...
        with _run_module() as (module, api, run):
            ...
            assert module.fail_json.called
            kwargs = module.fail_json.call_args[1]
            assert 'Failed to get current VDC certificate chain' in kwargs['msg']
...
class TestVdcCertificateEmptyChainPaths:
...
class TestVdcCertificatePostPutVerification:
...
class TestVdcCertificateNoLogSecurity:
    ...
    def test_no_log_on_private_key_content(self):
        ...
        with patch(f'{MODULE}.AnsibleModule', side_effect=capture_am), \
             ...
             patch(f'{MODULE}.VdcKeystoreApi') as mock_api_cls:
            mock_api = MagicMock()
...
class TestVdcCertificateMutuallyExclusive:
...
class TestVdcCertificateNoClientLibrary:
...
class TestVdcCertificateTimeout:
    """Test timeout parameter."""

    def test_timeout_param_propagates(self):
        """Timeout is passed through to VdcKeystoreApi."""
        from ansible_collections.dellemc.objectscale.plugins.modules.vdc_certificate import main
...
class TestEnsureClientStubCompatibility:
    ...
    def test_compat_secret_str(self):
        from ansible_collections.dellemc.objectscale.plugins.modules.vdc_certificate import _ensure_client_stub_compatibility
