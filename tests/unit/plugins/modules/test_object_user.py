...
class TestSyncTags:
    ...
    def test_check_mode_returns_changed_without_api_calls(self):
        obj = make_obj()
        current = [{'name': 'env', 'value': 'dev'}]
        desired = [{'name': 'env', 'value': 'prod'}]
        changed = obj.sync_tags('alice', 'ns1', current, desired, purge_tags=True, check_mode=True)
        assert changed is True
        # No API calls should be made in check mode
        obj.user_mgmt_api.user_management_service_add_user_tag.assert_not_called()
        obj.user_mgmt_api.user_management_service_update_user_tag.assert_not_called()
        obj.user_mgmt_api.user_management_service_remove_user_tags.assert_not_called()

    def test_check_mode_noop_when_equal(self):
        obj = make_obj()
        current = [{'name': 'env', 'value': 'prod'}]
        desired = [{'name': 'env', 'value': 'prod'}]
        changed = obj.sync_tags('alice', 'ns1', current, desired, purge_tags=True, check_mode=True)
...
class TestSyncLock:
    ...
    def test_check_mode_returns_changed_without_api_calls(self):
        obj = make_obj()
        changed = obj.sync_lock('alice', 'ns1', current_locked=False, desired_locked=True, check_mode=True)
        assert changed is True
        # No API call should be made in check mode
        obj.user_mgmt_api.user_management_service_set_user_lock.assert_not_called()

    def test_check_mode_noop_when_same(self):
        obj = make_obj()
        changed = obj.sync_lock('alice', 'ns1', current_locked=False, desired_locked=False, check_mode=True)
        assert changed is False
...
class TestSyncSecretKeys:
    ...
    def test_check_mode_create_returns_changed_without_api_calls(self):
        obj = make_obj()
        obj._list_existing_secret_keys = MagicMock(return_value=[])
        changed, created = obj.sync_secret_keys(
            'alice', 'ns1', [{'state': 'present'}], check_mode=True
        )
        assert changed is True
        # No API call should be made in check mode
        obj.secret_key_api.user_secret_key_service_create_new_key_for_user.assert_not_called()

    def test_check_mode_delete_returns_changed_without_api_calls(self):
        obj = make_obj()
        obj._list_existing_secret_keys = MagicMock(return_value=[{'secret_key_id': 'abc'}])
        changed, created = obj.sync_secret_keys(
            'alice', 'ns1', [{'state': 'absent', 'secret_key_id': 'abc', 'secret_key': 's3cr3t'}], check_mode=True
        )
        assert changed is True
        # No API call should be made in check mode
        obj.secret_key_api.user_secret_key_service_delete_key_for_user.assert_not_called()

    def test_check_mode_noop_when_key_present_and_desired_present(self):
        obj = make_obj()
        obj._list_existing_secret_keys = MagicMock(return_value=[{'secret_key_id': 'abc'}])
        changed, created = obj.sync_secret_keys(
            'alice', 'ns1', [{'state': 'present', 'secret_key_id': 'abc'}], check_mode=True
        )
        assert changed is False
...
class TestPerformModuleOperation:
    ...
    def test_check_mode_secret_keys(self):
        ...
        obj.perform_module_operation()
        obj.sync_secret_keys.assert_called_once_with('alice', 'ns1', [{'state': 'present'}], check_mode=True)
        assert obj.module.exit_json.call_args[1].get('changed') is True
    ...
    def test_create_with_locked_true_calls_sync_lock(self):
        ...
        obj.perform_module_operation()
        obj.create_user.assert_called_once()
        obj.sync_lock.assert_called_once_with('alice', 'ns1', False, True, check_mode=False)
    ...
    def test_create_with_locked_false_calls_sync_lock(self):
        ...
        obj.perform_module_operation()
        obj.create_user.assert_called_once()
        obj.sync_lock.assert_called_once_with('alice', 'ns1', False, False, check_mode=False)

    def test_create_without_locked_does_not_call_sync_lock(self):
        obj = make_obj(params={**BASE_PARAMS, 'state': 'present', 'locked': None})
        obj.get_user_details = MagicMock(side_effect=[None, {'name': 'alice', 'namespace': 'ns1', 'locked': False, 'tag': []}])
        obj.create_user = MagicMock()
        obj.sync_lock = MagicMock()
        obj.sync_secret_keys = MagicMock(return_value=(False, []))
        obj.perform_module_operation()
        obj.create_user.assert_called_once()
        obj.sync_lock.assert_not_called()

    def test_check_mode_tags(self):
        obj = make_obj(params={**BASE_PARAMS, 'state': 'present', 'tags': [{'name': 'env', 'value': 'prod'}]}, check_mode=True)
        obj.get_user_details = MagicMock(return_value={'name': 'alice', 'namespace': 'ns1', 'locked': False, 'tag': []})
        obj.sync_tags = MagicMock()
        obj.sync_secret_keys = MagicMock(return_value=(False, []))
        obj.perform_module_operation()
        obj.sync_tags.assert_called_once()
        assert obj.module.exit_json.call_args[1].get('changed') is True

    def test_check_mode_lock(self):
        obj = make_obj(params={**BASE_PARAMS, 'state': 'present', 'locked': True}, check_mode=True)
        obj.get_user_details = MagicMock(return_value={'name': 'alice', 'namespace': 'ns1', 'locked': False, 'tag': []})
        obj.sync_lock = MagicMock()
        obj.sync_secret_keys = MagicMock(return_value=(False, []))
        obj.perform_module_operation()
