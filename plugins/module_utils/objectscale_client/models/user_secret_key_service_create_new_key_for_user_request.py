...
class UserSecretKeyServiceCreateNewKeyForUserRequest(BaseModel):
    """UserSecretKeyServiceCreateNewKeyForUserRequest"""
    existing_key_expiry_time_mins: Optional[StrictStr] = Field(
        default=None,
        description="Expiry time in minutes for the secret key. Note that nodes may cache secret keys for up to two minutes so old keys  may not expire immediately.")  # noqa: E501
    namespace: Optional[StrictStr] = Field(default=None, description="Namespace for User qualifier if User Scope is NAMESPACE")
    secretkey: Optional[StrictStr] = Field(default=None, description="Secret key associated with this user. If not provided, system will generate one")
