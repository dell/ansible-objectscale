...
class ObjectCertificateServiceSetKeyCertificatePairRequest(BaseModel):
    """Request payload for PUT /object-cert/keystore — rotate_keycertchain."""
    ip_addresses: Optional[List[StrictStr]] = Field(
        default=None,
        description="Key and certificate parameter IP addresses. "
                    "Taken into account only if system_selfsigned is true.")
    system_selfsigned: Optional[StrictBool] = Field(
        default=None,
        description="Set true if the new certificate is self signed, false otherwise.")
    key_and_certificate: ObjectCertificateServiceSetKeyCertificatePairRequestKeyAndCertificate = Field(
        description="Key and certificate pair to set on the object-cert keystore.")
