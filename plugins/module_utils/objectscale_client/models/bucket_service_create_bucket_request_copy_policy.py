# coding: utf-8

"""
    Dell ObjectScale/ECS Management API

    Generated from ECS source code metadata. Provides reliable schemas and examples for client generation.

    The version of the OpenAPI document: 4.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


from __future__ import (absolute_import, division, print_function)
__metaclass__ = type

from typing import Any, Dict, Optional

try:
    from pydantic import BaseModel
except ImportError:
    from ansible_collections.dellemc.objectscale.plugins.module_utils.objectscale_client._stubs import BaseModel  # stub
try:
    from pydantic import ConfigDict
except ImportError:
    from ansible_collections.dellemc.objectscale.plugins.module_utils.objectscale_client._stubs import ConfigDict  # stub
try:
    from pydantic import StrictBool
except ImportError:
    from ansible_collections.dellemc.objectscale.plugins.module_utils.objectscale_client._stubs import StrictBool  # stub
try:
    from pydantic import StrictInt
except ImportError:
    from ansible_collections.dellemc.objectscale.plugins.module_utils.objectscale_client._stubs import StrictInt  # stub
try:
    from pydantic import StrictStr
except ImportError:
    from ansible_collections.dellemc.objectscale.plugins.module_utils.objectscale_client._stubs import StrictStr  # stub
try:
    from typing_extensions import Self
except ImportError:
    try:
        from typing import Self  # Python 3.11+
    except ImportError:
        from ansible_collections.dellemc.objectscale.plugins.module_utils.objectscale_client._stubs import Self  # stub


class BucketServiceCreateBucketRequestCopyPolicy(BaseModel):
    """BucketServiceCreateBucketRequestCopyPolicy"""
    days_after_last_write: Optional[StrictInt] = None
    minimum_size: Optional[StrictInt] = None
    external_endpoint: Optional[StrictStr] = None
    target_region: Optional[StrictStr] = None
    target_bucket: Optional[StrictStr] = None
    target_access_key: Optional[StrictStr] = None
    target_secret_key: Optional[StrictStr] = None
    target_account: Optional[StrictStr] = None
    target_role: Optional[StrictStr] = None
    tag_filter: Optional[StrictStr] = None
    policy_type: Optional[StrictStr] = None
    backup_read_target: Optional[StrictBool] = None
    sse_s3_enabled: Optional[StrictBool] = None
    detailed_log_enabled: Optional[StrictBool] = None
    detailed_log_errors_only: Optional[StrictBool] = None
    detailed_log_bucket: Optional[StrictStr] = None
    detailed_log_prefix: Optional[StrictStr] = None
    external_certs: Optional[StrictStr] = None

    model_config = ConfigDict(
        validate_by_name=True,
        validate_by_alias=True,
        protected_namespaces=(),
    )

    def to_dict(self) -> Dict[str, Any]:
        return self.model_dump(by_alias=True, exclude_none=True)

    @classmethod
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        if obj is None:
            return None
        return cls.model_validate(obj)
