...
class IamUser(object):
    ...
    def _handle_absent_delete(self, user_name, user, result):
        """Perform actual deletion for state=absent."""
        before_state = self.capture_current_state(user_name, user) if self.module._diff else None
    ...
    def _handle_absent(self, user_name, result):
        ...
        if user:
            if self.module.check_mode:
                ...
                self._handle_absent_delete(user_name, user, result)
    ...
    def _apply_present_subresources(self, user_name, user, result):
        ...
        self._apply_user_attributes(user_name, user, result)
        self._apply_user_policies(user_name, result)

    def _apply_access_key_delete(self, user_name, access_key_id, result):
        """Handle access key deletion."""
        if not self.module.check_mode:
            if self.delete_access_key(user_name, access_key_id):
                result['changed'] = True
        else:
            if any(k['AccessKeyId'] == access_key_id for k in self.list_access_keys(user_name)):
                result['changed'] = True

    def _apply_access_key_update(self, user_name, access_key_id, access_key_status, result):
        """Handle access key status update."""
        if not self.module.check_mode:
            if self.update_access_key_status(user_name, access_key_id, access_key_status):
                result['changed'] = True
        else:
            current_key = next((k for k in self.list_access_keys(user_name) if k['AccessKeyId'] == access_key_id), None)
    ...
    def _apply_access_key_ops(self, user_name, result):
        ...
        if access_key_state == 'present':
            ...
            self._apply_access_key_delete(user_name, access_key_id, result)
