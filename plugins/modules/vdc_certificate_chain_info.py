...
"""Ansible info module for querying the ObjectScale Object-cert keystore certificate chain."""
...
DOCUMENTATION = r'''
---
module: vdc_certificate_chain_info

version_added: '1.1.0'

short_description: Read the Object-cert keystore certificate chain on Dell ObjectScale

description:
- Returns the current Object-cert keystore certificate chain via
  GET /object-cert/keystore.
- This module queries the ObjectCertificateService endpoint, which is
  distinct from the VDCKeystoreService endpoint used by
  M(dellemc.objectscale.vdc_certificate_info).
...
- Supports check mode.
- This module queries the Object-cert (S3/object) certificate chain. For the
  VDC management certificate, use M(dellemc.objectscale.vdc_certificate_info).
...
EXAMPLES = r'''
- name: Get current Object-cert certificate chain
...
- name: Display Object-cert chain details
  ansible.builtin.debug:
    msg: |
      Leaf Subject: {{ chain_info.vdc_certificate_chain_details.leaf_subject | default('unknown') }}
      Expires: {{ chain_info.vdc_certificate_chain_details.not_after | default('unknown') }}
      Fingerprint: {{ chain_info.vdc_certificate_chain_details.fingerprint }}
      Chain Length: {{ chain_info.vdc_certificate_chain_details.chain_length }}

- name: Check Object-cert chain for compliance audit
  dellemc.objectscale.vdc_certificate_chain_info:
    objectscale_host: "{{ objectscale_host }}"
    objectscale_username: "{{ objectscale_username }}"
    objectscale_password: "{{ objectscale_password }}"
  register: audit_chain

- name: Warn if certificate metadata is unavailable
  ansible.builtin.debug:
    msg: "Install cryptography library for full certificate metadata"
  when: audit_chain.vdc_certificate_chain_details.leaf_subject is not defined
'''
...
vdc_certificate_chain_details:
    description: Current Object-cert keystore certificate details.
    returned: success
    type: dict
    contains:
        fingerprint:
...
class VdcCertificateChainInfo(object):
    """Gather Object-cert keystore certificate chain details from ObjectScale."""
    ...
    def __init__(self):
        ...
        try:
            ...
            self.keystore_api = ObjectCertKeystoreApi(self.api_client, timeout=timeout)
    ...
    def perform_module_operation(self):
        try:
            ...
            if str(status) == '404':
                self.module.exit_json(
                    failed=True,
                    msg="FC-224: Object-cert keystore not found at endpoint /object-cert/keystore",
                )
                return
            if str(status) == '401':
                self.module.exit_json(
            ...
            self.module.exit_json(
                failed=True,
                msg="Failed to get Object-cert certificate chain: %s" % error_msg,
            )
            return
        ...
        self.module.exit_json(
            changed=False,
            vdc_certificate_chain_details=details,
        )
