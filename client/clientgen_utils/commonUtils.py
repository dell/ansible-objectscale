...
def _filter_by_paths(json_obj, paths):
    ret = {}
    for key in json_obj:
        if any(key == path or (path.endswith('*')
               and key.startswith(path[:-1])) for path in paths):
...
def _get_all_required_models(all_models: dict, top_level_models: set) -> dict:
    ...
    # Find the redundant keys by taking the difference between all keys and
    # the references
...
def ProcessOpenapiSpec(file_path, paths):
    ...
    print("The number of models is: ", len(
        json_obj['components']['schemas'].keys()))
