...
def main() -> None:
    ...
    for py_file in sorted(target.rglob('*.py')):
        # Manually maintained files should not be re-processed.
        if py_file.name in ['_stubs.py', 'api_client.py', 'api_response.py', 'configuration.py', 'exceptions.py', 'rest.py']:
            continue
        # Root __init__.py is manually maintained (api/ and models/ __init__.py are managed by this script)
        if py_file.name == '__init__.py' and py_file.parent.name not in ['api', 'models']:
            continue
